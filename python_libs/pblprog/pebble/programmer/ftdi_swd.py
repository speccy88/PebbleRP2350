...
class FTDISerialWireDebug(object):
    def __init__(
        self, vid, pid, interface, direction, output_mask, reset_mask, frequency
    ):
        ...
        try:
            self._ftdi.open_mpsse(
                vid, pid, interface, direction=direction, frequency=frequency, latency=1
            )
        except:
            self._ftdi = None
            raise
    ...
    def reset_lo(self):
        direction = self._direction & ~(
            0x00 if self._output_enabled else self._output_mask
        )
