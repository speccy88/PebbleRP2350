...
class STM32FlashProgrammer(object):
    ...
    def erase_flash(self, flash_offset, length):
        ...
        for sector in erase_sectors:
            ...
            while (
                self._driver.read_memory_address(self.FLASH_SR_ADDR) & self.FLASH_SR_BSY
            ):
                time.sleep(0)
    ...
    def _wait_loader_state(self, wanted_state, timeout=3):
        ...
        while time.time() < end_time:
            ...
            raise Exception(
                "Timed out waiting for loader state %d, got %d" % (wanted_state, state)
            )
    ...
    def execute_loader(self):
        ...
        self._driver.write_memory_bulk(
            self.SRAM_BASE_ADDR, array.array("B", loader_bin)
        )
    ...
    def load_bin(self, offset, data):
        ...
        if actual_crc != expected_crc:
            raise Exception(
                "Bad CRC, expected %d, found %d" % (expected_crc, actual_crc)
            )
    ...
    def profile(self, duration):
        ...
        self._driver.write_memory_address(
            self.DEMCR_ADDR, demcr_value | self.DEMCR_TRCENA
        )
