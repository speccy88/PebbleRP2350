...
def wrap_cprogram_task_class():
    ...
    emx_cls = type(
        ...
        (orig_cls,),
        {
            ...
            "vars": ["EMCC_DEBUG", "EMCC_CORES", "EMCC_SETTINGS", "LINKDEPS"],
            "color": "ORANGE",
        },
    )
...
def add_cache_primer_node_if_needed(bld, env):
    ...
    existing_primer_node = cache_primer_node_by_cache_path.get(cache_path, None)
    ...
    bld.program(source=[source_node], target=primer_node, env=primer_env)
...
def process_emscripten_cprogram_link_args(self):
    ...
    transform_js_node_and_args = get_single_rule_or_env_value(
        "emx_transform_js_node_and_args"
    )
    ...
    if exported_functions:
        ...
        add_emcc_settings("-s", "EXPORTED_FUNCTIONS=@{}".format(exported_func_path))
...
def configure(conf):
    ...
    Logs.pprint("YELLOW", "Emscripten path is {}".format(conf.env.EMSCRIPTEN_ROOT))
    conf.env.CC = "emcc"
