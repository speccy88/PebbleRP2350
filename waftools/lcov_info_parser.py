...
class LcovInfoFileRecord(object):
    ...
    def __init__(self, file_path, root_to_strip=None, waf_bld=None):
        ...
        try:
            self.coverage_list = [
                "N" for i in range(sum(1 for line in open(file_path)))
            ]
        except IOError:
            fail(
                "Failed to open source file path to count total number of lines: %s"
                % file_path,
                waf_bld=waf_bld,
            )
    ...
    def process_da_line_info(self, da_line_info):
        ...
        if len(da_line_info_data) != 2:
            print(
                "Skipping lcov.info da line data due to parsing failure: %s"
                % da_line_info
            )
            return
        ...
        for line in lcov_info_file.read().splitlines():
            ...
            if line == "end_of_record":
                if current_file_record is None:
                    fail(
                        'Saw "end_of_record" before start of a file record',
                        waf_bld=waf_bld,
                    )
                # "end_of_record" denotes the end of a record, so add the record to our results
                coverage_results.append(
                    current_file_record.get_arcanist_coverage_dictionary()
                )
                # Reset our data
                current_file_record = None
            else:
                ...
                if info_type == "SF":
                    if current_file_record is not None:
                        fail(
                            "Saw start of new file record before previous file record ended",
                            waf_bld=waf_bld,
                        )
                    current_file_record = LcovInfoFileRecord(
                        info, root_to_strip=root_to_strip, waf_bld=waf_bld
                    )
                elif info_type == "DA":
                    if current_file_record is None:
                        fail(
                            "Saw line data before a file record started",
                            waf_bld=waf_bld,
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root_to_strip", type=str, help="Root to strip from front of file paths"
    )
    parser.add_argument("lcov_info_file", type=str, help="Path to lcov.info file")
    args = parser.parse_args()

    print(
        parse_lcov_info_for_arcanist(
            args.lcov_info_file, root_to_strip=args.root_to_strip
        )
    )
