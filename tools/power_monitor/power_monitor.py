...
class MatplotlibCurrentGraph:
    ...
    def graph_function(self, read_input, count):
        ...
        if count % self.samples_to_batch == 0:
            print(
                "%.3f secs to collect last %d samples"
            ...
            avg_last_set = "Last Collection Avg: %.3f uA" % (
                self.np.average(self.graph_data[-100:])
            )
            self.plt.text(
                0.9,
                0.9,
            ...
            self.plt.draw()
            self.plt.pause(0.01)  # a brief stall of the UI thread lets you zoom in, etc
            self.start_time = time.time()
    ...
    def graph_function(self, read_input, count):
        ...
        if count % self.samples_to_batch == 0:
            ...
            avg_last_set = "Last Collection Avg: %.3f mA            %s" % (
                self.np.average(self.graph_data[-100:]) / 1000,
                avg_all,
            )

            self.ds.data["y"] = list(self.graph_data)
            self.ds.data["x"] = self.np.linspace(
                1, len(self.graph_data), len(self.graph_data)
            )
            self.ds._dirty = True
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--vid", help="FTDI USB Vendor ID", default=0x0403, type=auto_int
    )
    parser.add_argument(
        "--pid", help="FTDI USB Product ID", default=0x6011, type=auto_int
    )
    ...
    parser.add_argument(
        "-c",
        "--continuous",
        help="Continuously monitor the current",
        action="store_true",
    )
    parser.add_argument(
        "--avg", action="store_true", help="calculate the average of the rails"
    )
    parser.add_argument(
        "-g", "--graph", help="locally graph power data", action="store_true"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        choices=ButtonNames,
        help="Push these buttons, release the rest. If this argument is omitted, the buttons are left as is",
    )
    ...
    parser.add_argument(
        "--usb_pwr", choices=["enable", "disable"], help="Turn on or off USB power"
    )
    parser.add_argument(
        "--acc_pu",
        choices=["enable", "disable"],
        help="enable/disable the accessory pull-up",
    )
    parser.add_argument(
        "-f",
        "--fast",
        help="Use libmpsse instead of pyftdi which collects samples ~6x faster at "
    ...
    parser.add_argument(
        ...
        help="Specify the platform being measured on.",
    )
    parser.add_argument(
        "--bokeh",
        action="store_true",
    ...
    if args.rails:
        ...
        def read_currents():
            ...
            read_tuple = [millis] + [
                PlatformRails[rail].readCurrent() for rail in args.rails
            ]
            return read_tuple
