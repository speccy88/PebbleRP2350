...
def writexml(workspace, fwusage, recfwusage):
    ...
    data_summary = analyze_static_memory_usage.analyze_sizes(
        workspace + "/build/src/fw/tintin_fw.elf"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        metavar="fwusage",
        required=True,
        help="current build FW usage: tintin.bin in kilobytes",
    )
    parser.add_argument(
        "--recfwusage",
        metavar="recfwusage",
        required=True,
        help="current recovery build FW usage: tintin.bin  in kilobytes",
    )
