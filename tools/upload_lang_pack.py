...
LP_URLS = {
    "staging": "https://lang-packs-staging.getpebble.com",
...
def upload_pack_to_s3(lang_pack):
    r = requests.get(
        "{}/admin/packs/sign".format(lp_url),
...
def post_edit_to_lp(pack_json):
    ...
    r = requests.post(
        "{}/admin/packs/{}/edit".format(lp_url, pack_json["id"]),
        data=post_data,
        auth=lp_auth,
    )
    assert_ok(r)
...
def create_new_pack_on_lp(pack_json):
    ...
    r = requests.post("{}/admin/packs/add".format(lp_url), data=post_data, auth=lp_auth)
    assert_ok(r)
...
def main():
    ...
    if pack_json is None:
        ...
        pack_json = create_new_pack_json(
            args.hardware, args.name, args.fw_version, metadata
        )
        is_new = True
