...
def get_pair(voltage_path):
    return [
        voltage_path,
        re.sub(
            "(\w+)%s.csv" % VOLTAGE_SUFFIX, "\\1%s.csv" % CURRENT_SUFFIX, voltage_path
        ),
    ]
...
def get_data(voltage_path, current_path):
    ...
    result["current"] = numpy.interp(
        result["time"], current["time"], current["current"]
    )
    return result
...
def get_curve(
    data,
    range=[100, 90, 80, 70, 60, 50, 40, 30, 20, 10, 5, 2, 0],
    is_discharge=False,
    capacity=0.0,
    graph=False,
):
    if "current" in data:
        ...
        # Use mAh as scale
        d_time = numpy.insert(numpy.diff(time), 0, 0) / 3600.0
    ...
    if is_discharge:
        ...
        if reverse[reverse_drop_index] < -10 and abs(drop_index - len(scale)) <= 3:
            ...
            print(
                "Detected end of charge! Dropping values past %ds @ %dmV."
                % (data["time"][drop_index], voltage[-1])
            )
    ...
    voltage = scipy.interpolate.InterpolatedUnivariateSpline(
        scale[threshold], avg[threshold], k=1
    )
...
def get_avg_curve(paths, range, is_discharge, graph, capacity):
    ...
    for voltage_csv, current_csv in parsed_paths:
        curve = get_curve(
            get_data(voltage_csv, current_csv), range, is_discharge, capacity, graph
        )
        if graph:
...
def main(argv):
    ...
    parser.add_argument("-o", "--output", help="Where should we write the results?")
    parser.add_argument(
        "-c",
        "--curve",
        default="100,90,80,70,60,50,40,30,20,10,5,2,0",
        help="What values of the curve do we calculate?",
    )
    parser.add_argument(
        "-d", "--discharge", action="store_true", help="Is this discharge data?"
    )
    parser.add_argument(
        "-g", "--graph", action="store_true", help="Should we show a graph?"
    )
    parser.add_argument(
        "-bc", "--capacity", default="0", help="What is the battery's capacity? (in mA)"
    )
