...
class ELFFileHeader(ELFObjectBase):
    ...
    sh_str_index = (
        None  # The section header table index of the section name string table entry
    )
...
class ELFSectionHeader(ELFObjectBase):
    ...
    def __repr__(self):
        ...
        return (
            ...
            "info={}, addr_align={}, entry_size={}>".format(
                self.__class__.__name__, hex(id(self)), *fields
            )
        )
...
class ELFSymbolTableEntry(ELFObjectBase):
    ...
    def __repr__(self):
        fields = (self.name, self.addr, self.size, self.info, self.other, self.sh_idx)
        return (
            "<{}@{}: name={}, addr={}, size={}, info={}, other={}, sh_idx={}>".format(
                self.__class__.__name__, hex(id(self)), *fields
            )
        )
...
class ELFFile(ELFObjectBase):
    ...
    def unpack(self, raw_data):
        ...
        for i in range(self.file_header.sh_num):
            ...
            header.unpack(
                raw_data,
                self.file_header.sh_offset + i * self.file_header.sh_entry_size,
            )
    ...
    def get_exported_symbols(exports, result, rev):
        for export in exports:
            if export["type"] == "function":
                if (
                    any(k in export for k in ["internal", "removed", "deprecated"])
                    or export["addedRevision"] > rev
                ):
                    continue
