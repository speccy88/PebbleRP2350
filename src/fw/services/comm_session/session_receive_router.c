...
#if defined(CONFIG_BOARD_FRUITJAM_RP2350)
static uint16_t prv_fruitjam_debug_length(uint32_t length) {
...
static bool prv_handle_endpoint_error_and_skip_message_if_needed(
    CommSession *session, const PebbleProtocolEndpoint *endpoint, const uint16_t endpoint_id) {
...
#if defined(CONFIG_BOARD_FRUITJAM_RP2350)
    fruitjam_bt_debug_record_pp_error(endpoint_id, meta_response_info.payload.error_code);
#endif
...
static bool prv_prepare_receiver(const uint32_t payload_length,
  ...
  if (!receiver) {
...
#if defined(CONFIG_BOARD_FRUITJAM_RP2350)
    fruitjam_bt_debug_record_pp_no_buffer(endpoint_id, prv_fruitjam_debug_length(payload_length));
#endif
    ...
    PBL_LOG_ERR("No receiver for endpoint=%" PRIu16 " len=%" PRIu32, endpoint_id, payload_length);
...
#if defined(CONFIG_BOARD_FRUITJAM_RP2350)
      fruitjam_bt_debug_record_pp_message(false, endpoint_id,
                                          prv_fruitjam_debug_length(payload_length));
#endif
